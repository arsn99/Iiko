	...
	def UseSebes()
		...
		if dataSebes['data'][1].blank?
			if  dataSebes['data'][0]['OpenDate.Typed'] == Date.today.strftime("%Y-%m-%d")
				dataSebes['data']<<{'ProductCostBase.Profit'=> dataSebes['data'][0]['ProductCostBase.Profit']}
				dataSebes['data'][0]['ProductCostBase.Profit']="Продаж нет"		
			else
				dataSebes['data']<<{'ProductCostBase.Profit'=> 0}	
			end
		end
		...
		# себе стоимость месяц
		dataSebesMounthPrecent = $iiko.IikoPostRequestForSebesMounth("PostForSebes%.json","CUSTOM")
